...
export const Transcript = ({ meetingId }: Props) => {
  ...
  const filteredData = (data ?? []).filter((item) => {
    const text = 'text' in item ? item.text : item.item.text;
    ...
    <ScrollArea>
        <div className="flex flex-col gap-y-4">
            {filteredData.map((item) => {
                const start_ts = 'start_ts' in item ? item.start_ts : item.item.start_ts;
                return (
                    <div
                        key={start_ts}
                    ...
                    >
                        <div className="flex gap-x-2 items-center">
                            ...
                            <p className="text-sm text-blue-500 font-medium">
                                {format(
                                    new Date(0, 0, 0, 0, 0, 0, start_ts), "mm:ss"
                                )}
                            </p>
                        </div>
                        <Highlighter
                            ...
                            autoEscape={true}
                            textToHighlight={'text' in item ? item.text : item.item.text}
                        />
                    </div>
                );
            })}
